...
// Sergio Bromberg Added Fractal Noise Shader Graph node.
...
float3 mod_3(float3 x, float3 y)
{
...
{
  float3 Pi0 = mod_3(floor(P), rep); // Integer part, modulo period
  float3 Pi1 = mod_3(Pi0 + (float3)1.0, rep); // Integer part + 1, mod period
  Pi0 = mod289_3(Pi0);
  Pi1 = mod289_3(Pi1);
...
void FractalNoise_float(float3 Coordinates, int Octaves, float FractalAmplitudeFactor, float FractalFrequencyFactor, out float Noise) 
{
	...
	for (int i = 0; i < Octaves; i++) {
		...
		float3 coord = Coordinates * s;
		float3 period = float3(s, s, s) * 2.0;
		
		float vx, vy, vz;
		float3 gradient;
		
		SimplexNoise3D_float(coord + float3(epsilon, 0, 0), vx, gradient);
		SimplexNoise3D_float(coord + float3(0, epsilon, 0), vy, gradient);
		SimplexNoise3D_float(coord + float3(0, 0, epsilon), vz, gradient);

		o += w * float3(vx, vy , vz);
		
		s *= FractalFrequencyFactor;
		w *= FractalAmplitudeFactor;
